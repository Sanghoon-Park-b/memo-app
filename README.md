...
### 1. 이슈 생성
- 작업 전에 한국어로 이슈를 먼저 생성하세요
- 이슈 템플릿을 사용하여 상세한 내용을 작성하세요

### 2. 브랜치 생성
- 브랜치명 형식: {type}/issue-number`n- 예시: eature/123, ix/456, hotfix/789`n
### 3. PR 생성
- 작업 완료 후 한국어로 PR을 생성하세요
- PR 템플릿을 사용하여 상세한 내용을 작성하세요

## 커밋 메시지 규칙

### 형식
- 한국어 명령조로 작성 (끝에 '~하라' 사용)
- 구조: 제목 (필수) + 설명 (선택사항)
- 제목 형식: [작업내용] [목적]하라`n
### 예시
- CLAUDE.md 파일 추가 및 PR 생성, 이슈, PR 템플릿 구조 규칙 정의하라`n- Android 네비게이션 바 테마 동적 변경 및 Safe Area 개선하라`n- 타입 정의 개선 및 코드 중복 제거하라`n- 웹뷰 컴포넌트 백그라운드 상태 관리 및 강제 새로고침 기능 추가하라`n
